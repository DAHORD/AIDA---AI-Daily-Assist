...
def get_app_data_path(filename=""):
    ...
    """
    # Utilise os.path.expanduser('~') + AppData pour être compatible multiplateforme
...
CONFIG_FILE = get_app_data_path("config.ini") # Nouvelle constante pour config.ini

def charger_cles_api():
    config = configparser.ConfigParser()
    
    # Utilisation de la constante CONFIG_FILE
    if not os.path.exists(CONFIG_FILE):
        console.print(f"[bold red]Erreur : Le fichier de configuration est introuvable à {CONFIG_FILE}.[/bold red]")
        return {}
...
def afficher_aide():
    table = Table(title="💎 Commandes Disponibles v2.9 💎 DAHORD", style="cyan", header_style="bold magenta")
    ...
    commandes_aide = {
        ...
        "--- Général ---": "",
        "lang": "Change la langue de l'assistant (fr/en).",
...
def process_command(command_line: str, commandes: dict, aliases: dict):
    ...
    if commande_resolue in ["q", "quit", "exit"]:
        console.print("[bold blue]À bientôt ! - DAHORD[/bold blue]")
        # Leger délai pour que le message soit lu
        time.sleep(1)
        return False # Arrête la boucle
    
    elif commande_resolue in commandes:
        try:
            if commande_resolue in ["todo","lang"]:
...
def handle_lang_command(args):
    ...
    # Doit utiliser la même logique que le launcher pour trouver le fichier
    try:
        app_data_root = os.path.join(os.path.expanduser('~'), 'AppData', 'Roaming')
        ...
        if new_lang == 'fr':
            console.print("[cyan]La langue est déjà le Français.[/cyan]")
        else:
            console.print(f"[bold green]Langue changée en '{new_lang}'. Veuillez redémarrer l'application.[/bold green]")
            
    except Exception as e:
        console.print(f"[red]Erreur lors du changement de langue : {e}[/red]")

def main():
    """Fonction principale pour l'exécution en mode terminal classique."""
    console.print(Panel("[bold green]🚀 Assistant de Terminal v2.9 - Mode Productivité 🚀 - Par DAHORD[/bold green]", border_style="green"))
    ...
    commandes = {
        ...
        "mdp": generer_mdp, "timer": compte_a_rebours, "blg": raconter_blague, "lang": handle_lang_command, "help": afficher_aide,
    }
